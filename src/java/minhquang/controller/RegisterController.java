...
public class RegisterController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            UserDTO googleUser = (UserDTO) session.getAttribute("USER");
            if(googleUser != null) {
            ...
            if(!password.equals(confirm)) {
                userError.setPassword("Password and Confirm Password Does not match");
                request.setAttribute("USER_ERROR", userError);
            } else {
                googleUser.setUsername(username);
                googleUser.setPassword(password);
                googleUser.setAddress(address);
                googleUser.setPhoneNo(phoneNo);
                
                UserDAO userDao = new UserDAO();
                if(userDao.signUp(googleUser)) {
